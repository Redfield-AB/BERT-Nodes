...
import se.redfield.bert.core.ZstcCommands;
...
public class ZeroShotTextClassifierNodeModel extends NodeModel {
	...
	private BufferedDataTable  runZeroShotTextClassifier(
				 BertModelConfig zstcModel, 
		...
		try (ZstcCommands commands = new ZstcCommands(settings.getPythonCommand())){
			...
			return commands.getDataTable(ZstcCommands.VAR_OUTPUT_TABLE, exec, exec.createSilentSubProgress(0.1));
	...
	private String getZeroShotTextClassifierScript(BertModelConfig zstcModel) {
		...
		DLPythonSourceCodeBuilder b = DLPythonUtils.createSourceCodeBuilder("from ZeroShotTextClassifier import ZeroShotTextClassifier");
	
		b.a(ZstcCommands.VAR_OUTPUT_TABLE).a(" = ZeroShotTextClassifier.run_zstc(").n();
		ZstcCommands.putInputTableArgs(b); 										// input_table
		ZstcCommands.putSentenceColumnArg(b, settings.getSentenceColumn());		// sentence_column
		ZstcCommands.putCandidateLabelsArg(b, settings.getCandidateLabels());	// candidate_labels
		ZstcCommands.putHypothesisArgs(b, settings.getHypothesis());			// hypothesis
		ZstcCommands.putZstcModelArgs(b, zstcModel);    						//bert_model_handle, cach_dir
